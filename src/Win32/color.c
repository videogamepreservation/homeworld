...
/*-----------------------------------------------------------------------------
    Name        : colRGBCompare
    Description : Compare 2 color buffers together.
    Inputs      : p0, p1 - buffers to compare
                  nPixels - number of pixels to compare
    Outputs     :
    Return      : 0 if they are exactly equal, nonzero otherwise
----------------------------------------------------------------------------*/
/*
sdword colRGBCompare(color *p0, color *p1, sdword nPixels)
{
    ...
    {
        if (((*p0) & 0x00ffffff) != ((*p1) & 0x00ffffff))
        {
            return(-1);
        }
    }
