 ...
 * Also there is another usage of this bit, in DX6 if app uses SetTexture() call
 * to bind a texture to a device, any modification to the texture surface
 * via BLT/LOCK/UNLOCK etc will reflect only on any rendering call thereafter
 * and will not cause ambigious state where the modification might also affect
 * any previous rendering call. However, this automatic syncronization comes 
 * with a cost in performance, therefore an app is given a chance to cut this 
 * overhead by specifying this bit during surface creation if this texture is 
 * known in advance that it's not going to be updated during begin/end scene.
 * Correctness can't be guranteed if a surface has this flag but gets updated
 * anyway by app between BeginScene and EndScene.
 */
