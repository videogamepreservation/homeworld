...
Sub Render()
    'Check flag
    If fMainForm.mnuViewGraphSide.Checked Then rendUpdateCont (nContext)
End Sub
...
Sub SetSel(ByVal bFlag As Boolean)
    'Check flag
    If Not fMainForm.mnuViewGraphSide.Checked Then Exit Sub
...
Sub SetView(ByVal bFlag As Boolean)
    ...
    'Set view
    Call rendSetContView(nContext, (-pbViewPort.Width / (fConvScale * 2)) - aOffset(0), (-pbViewPort.Height / (fConvScale * 2)) + aOffset(1), pbViewPort.Width / fConvScale, pbViewPort.Height / fConvScale)
...
Sub Reset()
    ...
    'Set side view position and size
...
Private Sub Form_Load()
    ...
    'Create new context
    If rendNewCont(nContext, pbViewPort.hWnd, 3) < 0 Then Call MsgBox("DLL error: Unable to create context!", vbOKOnly Or vbExclamation, "MissionMan")
    ...
    'Get window
    Call misGetListByKey(MIS_SEC_COM, MIS_KEY_SIDEV, sList, nCount, MIS_MOD_INI)
    ...
    On Error GoTo 0
    fMainForm.mnuViewGraphSide.Checked = True
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    'Cleanup form
    fMainForm.mnuViewGraphSide.Checked = False
    ...
    'Put window
    Call misPutListByKey(MIS_SEC_COM, MIS_KEY_SIDEV, sList, MIS_MOD_INI)
End Sub
...
Private Sub pbViewPort_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If bClick = True Then
        ...
        If nButton = 1 And nShift = 1 Then
            'Set offset
            aOffset(0) = aOffset(0) + (X - fMx) / fConvScale
        ...
        If nButton = 2 And nShift = 2 Then
            ...
            If bCamFlag = 1 Then
                'Check selection
                If nSel = 1 Then
                    aEye(0) = aEye(0) + (X - fMx) / (fConvScale * fViewScale)
                    aEye(1) = aEye(1) + (fMy - Y) / (fConvScale * fViewScale)
                End If
                
                'Check selection
                If nSel = 2 Then
                    aFocus(0) = aFocus(0) + (X - fMx) / (fConvScale * fViewScale)
        ...
        If nButton = 1 And nShift = 4 Then
            'Translate object
            Call rendTransSel((X - fMx) / (fConvScale * fViewScale), (fMy - Y) / (fConvScale * fViewScale), 0)
        ...
        If nButton = 2 And nShift = 4 Then
            ...
            Else
                'Rotate selection
                Call rendRotSel(nContext, 0, 0, fA)
            End If
                
            'Update status bar
            sbStatusBar.Panels.Item(1).Text = "Object Rotation: x" + " 0" + " y" + " 0" + " z" + Format(fAng, " 0; -#") + " deg"
    ...
    'Get cursor
    aMouse(0) = (((X - pbViewPort.Width / 2) / fConvScale) - aOffset(0)) / fViewScale
...
Private Sub pbViewPort_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If nButton = 2 And nShift = 4 Then
        ...
        If bRotFlag = 1 Then
            ...
            If InStr(sListKey, " ") = 0 Then
                'Rotate selection
                Call rendRotSel(0, 0, 0, fAng)
            Else
                'Rotate selection
                Call rendRotSel(nContext, 0, 0, fAng)
            End If
        End If
    ...
    If nButton = 1 And nShift = 3 Then
        'Get cursor
        aCursor(0) = (((X - pbViewPort.Width / 2) / fConvScale) - aOffset(0)) / fViewScale
