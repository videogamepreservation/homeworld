...
Sub Render()
    'Check flag
    If fMainForm.mnuViewGraphTop.Checked Then Call rendUpdateCont(nContext)
End Sub
...
Sub SetSel(ByVal bFlag As Boolean)
    'Check flag
    If Not fMainForm.mnuViewGraphTop.Checked Then Exit Sub
...
Sub SetView(ByVal bFlag As Boolean)
    ...
    'Set view
    Call rendSetContView(nContext, (-pbViewPort.Width / (fConvScale * 2)) + aOffset(2), (-pbViewPort.Height / (fConvScale * 2)) + aOffset(0), pbViewPort.Width / fConvScale, pbViewPort.Height / fConvScale)
...
Sub Reset()
    ...
    'Set top view position and size
...
Private Sub Form_Load()
    ...
    'Create new context
    If rendNewCont(nContext, pbViewPort.hWnd, 2) < 0 Then Call MsgBox("DLL error: Unable to create context!", vbOKOnly Or vbExclamation, "MissionMan")
    ...
    'Get window
    Call misGetListByKey(MIS_SEC_COM, MIS_KEY_TOPV, sList, nCount, MIS_MOD_INI)
    ...
    On Error GoTo 0
    fMainForm.mnuViewGraphTop.Checked = True
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    'Cleanup form
    fMainForm.mnuViewGraphTop.Checked = False
    ...
    'Put window
    Call misPutListByKey(MIS_SEC_COM, MIS_KEY_TOPV, sList, MIS_MOD_INI)
End Sub
...
Private Sub Form_KeyPress(KeyAscii As Integer)
    ...
    If (KeyAscii = Asc("-")) Or (KeyAscii = Asc("+")) Then
        ...
        aOffset(0) = aOffset(0) * fViewScale / fScale
...
Private Sub pbViewPort_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    Dim fZ As Single
    Dim fA As Single
    Dim fOrigX As Single
    Dim fOrigY As Single
    Dim fOrigZ As Single
    Dim fDist As Single
    Dim aBand(4) As Single
    ...
    If bClick = True Then
        ...
        If nButton = 1 And nShift = 1 Then
            ...
            aOffset(0) = aOffset(0) + (fMy - Y) / fConvScale
        ...
        If nButton = 3 And nShift = 1 Then
            ...
            aOffset(0) = aOffset(0) / fViewScale
        ...
        If nButton = 2 And nShift = 2 Then
            ...
            If bCamFlag = 1 Then
                'Check selection
                If nSel = 1 Then
                    aEye(0) = aEye(0) + (fMy - Y) / (fConvScale * fViewScale)
                    aEye(2) = aEye(2) + (fMx - X) / (fConvScale * fViewScale)
                End If
                
                'Check selection
                If nSel = 2 Then
                    aFocus(0) = aFocus(0) + (fMy - Y) / (fConvScale * fViewScale)
        ...
        If nButton = 1 And nShift = 4 Then
            'Translate object
            Call rendTransSel((fMy - Y) / (fConvScale * fViewScale), 0, (fMx - X) / (fConvScale * fViewScale))
        ...
        If nButton = 2 And nShift = 4 Then
            ...
            Else
                'Rotate selection
                Call rendRotSel(nContext, 0, fA, 0)
            End If
                
            'Update status bar
            sbStatusBar.Panels.Item(1).Text = "Object Rotation: x" + " 0" + " y" + Format(fAng, " 0; -#") + " z" + " 0" + " deg"
        ...
        frmFront.SetDetail (0)
        frmSide.SetDetail (0)
    End If
    
    'Get cursor
    aMouse(0) = -(((Y - pbViewPort.Height / 2) / fConvScale) + aOffset(0)) / fViewScale
    aMouse(1) = aCursor(1)
    ...
    If bClick = False Then
        ...
        If fMainForm.mnuViewGraphFront.Checked Then frmFront.sbStatusBar.Panels.Item(1).Text = "Cursor: x" + Format(aMouse(0), " 0; -#") + " y" + Format(aMouse(1), " 0; -#") + " z" + Format(aMouse(2), " 0; -#") + " m"
...
Private Sub pbViewPort_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If nButton = 2 And nShift = 4 Then
        ...
        If bRotFlag = 1 Then
            ...
            If InStr(sListKey, " ") = 0 Then
                'Rotate selection
                Call rendRotSel(0, 0, fAng, 0)
            Else
                'Rotate selection
                Call rendRotSel(nContext, 0, fAng, 0)
            End If
        End If
    ...
    If nButton = 1 And nShift = 3 Then
        'Get cursor
        aCursor(0) = -(((Y - pbViewPort.Height / 2) / fConvScale) + aOffset(0)) / fViewScale
        aCursor(2) = -(((X - pbViewPort.Width / 2) / fConvScale) + aOffset(2)) / fViewScale
        
        'Check grid flag
        If bGridFlag = 1 Then
            'Snap cursor x to grid
            If Abs(aCursor(0)) Mod fGridSize < fGridSize / 2 Then
                aCursor(0) = -(aCursor(0) Mod fGridSize) + aCursor(0)
            Else
                aCursor(0) = Sgn(aCursor(0)) * fGridSize - (aCursor(0) Mod fGridSize) + aCursor(0)
            End If
