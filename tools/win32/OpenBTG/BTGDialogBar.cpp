...
void CBTGVBackgroundBar::OnInit(void)
{
	...
	CEdit *eRed, *eGreen, *eBlue, *eAlpha;
	char buffer[20];
    ...
    sAlpha = (CSliderCtrl *)GetDlgItem(ID_BGALPHASLIDER);
...
void CBTGVBackgroundBar::OnUpdate(void)
{
	...
	CEdit *eRed, *eGreen, *eBlue, *eAlpha;
	char buffer[20];
	CString tRed, tGreen, tBlue, tAlpha;
	...
	// Has the edit changed?
	else if((atoi(tAlpha) != alpha) && (sAlpha->GetPos() == blue))
	{
		alpha = atoi(tAlpha);
		bUpdate = 1;
