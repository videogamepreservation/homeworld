...
BEGIN_MESSAGE_MAP(CBTGView, CView)
    ...
    ON_COMMAND(ID_BTG_LIGHTMODE, OnBtgLightmode)
...
void CBTGView::OnDraw(CDC* pDC)
{
    ...
    if (pDoc->btgDocMode == CBTGDoc::btgDocMode_EditBTGLights)
    {
        btglRDraw(pDoc);
    }
...
void CBTGView::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	case CBTGDoc::btgDocMode_EditBTGLights:
		// Did we click on an existing light?
		if(pDoc->btglCheckExisting(point))
		{
			// Is it selected?
			if(!bitTest(pDoc->btglCheckExisting(point)->flags, BTGLight::btglFlags_Selected))
			{
				// New click.  Clear selection.
				pDoc->btglClearSelected();
			}
			
			// Yes.  Select it.
			pDoc->btglSelect(point);
...
void CBTGView::OnBtgLightmode()
{
    CBTGDoc* pDoc = GetDocument();
    pDoc->btgDocMode = CBTGDoc::btgDocMode_EditBTGLights;
    pDoc->EverythingClearSelected();
    Invalidate();
}
