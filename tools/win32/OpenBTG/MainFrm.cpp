...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
    ...
    ON_UPDATE_COMMAND_UI(ID_BTG_LIGHTMODE, OnUpdateBtgLightmode)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_wndBackgroundBar.EnableDocking(CBRS_ALIGN_RIGHT);
	m_wndVisibleBar.EnableDocking(CBRS_ALIGN_RIGHT);
	m_wndBitmapBar.EnableDocking(CBRS_ALIGN_RIGHT);
...
BOOL CMainFrame::PreTranslateMessage(MSG* pMsg) 
{
	...
	pDoc->btgvUpdateBackgroundColor(m_wndBackgroundBar.red, m_wndBackgroundBar.green, m_wndBackgroundBar.blue, m_wndBackgroundBar.alpha);

    char buf[64];
    pDoc->GetLightString(buf);
    if (buf[0])
    {
...
void CMainFrame::OnUpdateBtgPolyCount(CCmdUI* pCmdUI)
{
	...
	else
	{
        ...
        case CBTGDoc::btgDocMode_EditBTGLights:
            m_wndStatusBar.SetPaneText(0, "Light Edit Mode");
            break;
...
void CMainFrame::OnUpdateBtgLightmode(CCmdUI* pCmdUI)
{
    btgDocMode = CBTGDoc::btgDocMode_EditBTGLights;
    gBTGDoc->btgbgReset();
    m_wndStatusBar.SetPaneText(0, "Light Edit Mode");
}
